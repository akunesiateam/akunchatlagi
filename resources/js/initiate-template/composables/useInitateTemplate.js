 ...
 */
export function useInitateTemplate(props) {
    // ===============================
    // REACTIVE STATE
    // ===============================
            ...
            tributeAttachedElements.forEach((el) => {
                if (el && el.parentNode) {
                    try {
                        ...
                        console.warn(
                            "Failed to detach tribute from element:",
                            e,
                        );
                    }
                }
    ...
    const loadContactsFromHidden = () => {
        const el = document.getElementById("initiate-contacts");

        if (el && el.value) {
            try {
                contacts.value = JSON.parse(el.value);
            } catch (e) {
    ...
    const save = async () => {
        ...
        // Add contact IDs - use contacts from hidden input or props
        const contactsToUse =
            contacts.value.length > 0 ? contacts.value : props.contacts || [];
        if (contactsToUse.length > 0) {
            contactsToUse.forEach((contact, index) => {
                // Handle both contact object {id: 1} or direct ID
                const contactId =
                    typeof contact === "object" ? contact.id : contact;
                formData.append(`contact_ids[${index}]`, contactId);
            });
        }
        ...
        try {
            const response = await fetch(
                `/${tenantSubdomain}/contacts/initiatechat`,
                {
                    method: "POST",
                    headers: {
                        "X-CSRF-TOKEN":
    ...
    const handleCancel = () => {
        ...
        window.dispatchEvent(
            new CustomEvent("close-modal", { detail: "initiate-template" }),
        );
    };
    ...
    const loadData = () => {
        ...
        loadMergeFields();
        loadContactsFromHidden(); // Load contacts from hidden input
    };
    ...
    document.addEventListener("initiate-chat-opened", () => {
        loadContactsFromHidden();
    });
